     ...
     <section>
          <div class="container">
               ...
               <div class="clearfix">
                    <?php
                    {
                         $job_id12 = $_POST['job_id'];
                         $qur = "select * from `done_exam` where `job_id`=$job_id12 and candidate_id=".$_SESSION['candidateid'];
                         $result_qur = mysqli_query($con,$qur);
                    }

                    {
                         $job_id123 = $_POST['job_id'];
                         $qur123 = "select * from `selected_candidate` where `job_id`=$job_id123 and candidate_id=".$_SESSION['candidateid'];
                         $result_qur123 = mysqli_query($con,$qur123);
                    }


                    if($r_qur123 = mysqli_num_rows($result_qur123) > 0){
                         echo '<h1>Congress You are selected !!!</h1>';
                    }
                    else if($r_qur = mysqli_num_rows($result_qur) > 0){
                         echo '<h1>You have already given the exam !!!</h1>';
                    }
                    else{
                         echo '<form action="exam.php" method="post">
                                   <input type="hidden" value="'.$m.'" name="job_id">
                                   <input type="hidden" value="'.$m1.'" name="employer_id">
                                                 
                                   <input type="submit" class="section-btn btn btn-primary pull-left" value="First, Give Exam" name="submit">
                              </form>';
                    }
